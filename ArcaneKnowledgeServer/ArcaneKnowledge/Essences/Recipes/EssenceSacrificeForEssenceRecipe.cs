...
namespace Eco.Mods.TechTree
{
    ...
    public class EssenceSacrificeForEssenceRecipe : RecipeFamily
    {
        // Settings:                                     Ratio, Xp, Time, Labor
        private static float[] Settings => new float[] { 0.01f, 0.1f, 1.6f, 60f };
        
        // Input
        private IngredientElement Ingredient => new IngredientElement(
            "Essence", SourceAmount, typeof(ArcaneKnowledgeSkill), typeof(ArcaneKnowledgeLavishReqTalent)
        );
        
        // Table
        private Type TableType => typeof(ArcaneKnowledgeCircleObject);

        // Output
        private CraftingElement CraftingOutput => new CraftingElement<ResearchEssenceItem>(OutputAmount);

        public EssenceSacrificeForEssenceRecipe()
        {
            InitializeRecipe();
        }
